...
class ControllerExtensionFeedRemarketingFeed extends Controller
{
    ...
    private function generateFeed()
    {
        ...
        if (!empty($this->request->get['currency'])) {
            $query = $this->db->query(
                "SELECT currency_id FROM `" . DB_PREFIX . "currency` WHERE code = '" . $this->db->escape(
        ...
        if (!empty($this->request->get['language'])) {
            $query = $this->db->query(
                "SELECT language_id FROM `" . DB_PREFIX . "language` WHERE code='" . $this->db->escape(
    ...
    private function getXml($page = 0)
    {
        ...
        $multiplier = (float)$this->config->get('remarketing_feed_multiplier') > 0 ? (float)$this->config->get(
            'remarketing_feed_multiplier'
        ) : 1;
        ...
        $option_color = $this->config->get('remarketing_feed_option_color') != '' ? explode(
            ',',
            $this->config->get('remarketing_feed_option_color')
        ) : [];
        $option_size = $this->config->get('remarketing_feed_option_size') != '' ? explode(
            ',',
            $this->config->get('remarketing_feed_option_size')
        ) : [];

        if (!empty($color)) {
            $allowed_attributes = array_merge($allowed_attributes, $color);
        }
        if (!empty($size)) {
            $allowed_attributes = array_merge($allowed_attributes, $size);
        }
        if (!empty($material)) {
            $allowed_attributes = array_merge($allowed_attributes, $material);
        }
        if (!empty($gender)) {
            $allowed_attributes = array_merge($allowed_attributes, $gender);
        }
        if (!empty($age_group)) {
            $allowed_attributes = array_merge($allowed_attributes, $age_group);
        }
        ...
        foreach ($this->model_tool_remarketing_core->getProducts($start, $limit, $ocstore) as $product) {
            ...
            if ($export_options && (!empty($option_color) || !empty($option_size))) {
                ...
                if (!empty($color_options) && empty($size_options)) {
                    foreach ($color_options as $color_option) {
                        if ($color_option['subtract'] && $color_option['quantity'] <= 0) {
                            continue;
                        }
                    ...
                    foreach ($size_options as $size_option) {
                        if ($size_option['subtract'] && $size_option['quantity'] <= 0) {
                            continue;
                        }
            ...
            foreach ($all_products as $product) {
                ...
                if (mb_strlen($product['description'], 'UTF-8') < 100) {
                    $product['description'] = str_replace(['{product_name}', '{product_model}'],
                ...
                if ($truncate_description) {
                    ...
                    if (mb_strlen($product['description']) > $limit) {
                        ...
                        $product['description'] = trim(
                            mb_substr($product['description'], 0, $lastspacePos),
                            '.,;&+-\//?!'
                        );
                    }
                }
                ...
                if ($original_description) {
                    $xml .= '<g:description><![CDATA[' . html_entity_decode(
                            $product['description'],
                            ENT_QUOTES
                        ) . ']]></g:description>' . $this->eol;
                } else {
                    $xml .= '<g:description><![CDATA[' . $this->checkField(
                            $product['description']
                        ) . ']]></g:description>' . $this->eol;
                }

                if ($rich_text) {
                    $xml .= '<g:rich_text_description><![CDATA[' . html_entity_decode(
                            $product['description']
                        ) . ']]></g:rich_text_description>' . $this->eol;
                }

                $link = $this->url->link('product/product', 'product_id=' . $product['product_id']);

                if ($utm) {
                    $utm_to_link = str_replace(['{product_id}', '{product_model}'],
                        [$product['product_id'], $product['model']],
                        $utm);
                ...
                if ($original_images) {
                    ...
                    if (version_compare(VERSION, '3.0.0.0', '>=')) {
                        $xml .= '<g:image_link>' . $this->model_tool_image->resize(
                                $product['image'],
                                $this->config->get(
                                    'theme_' . $this->config->get('config_theme') . '_image_popup_width'
                                ),
                                $this->config->get(
                                    'theme_' . $this->config->get('config_theme') . '_image_popup_height'
                                )
                            ) . '</g:image_link>' . $this->eol;
                    } else {
                        $xml .= '<g:image_link>' . $this->model_tool_image->resize(
                                $product['image'],
                ...
                if (!empty($this->product_images[$product['product_id']])) {
                    foreach ($this->product_images[$product['product_id']] as $image) {
                        if ($original_images) {
                            ...
                            if (version_compare(VERSION, '3.0.0.0', '>=')) {
                                $xml .= '<g:additional_image_link>' . $this->model_tool_image->resize(
                                        $image,
                                        $this->config->get(
                                            'theme_' . $this->config->get('config_theme') . '_image_popup_width'
                                        ),
                                    ...
                                    ) . '</g:additional_image_link>' . $this->eol;
                            } else {
                                $xml .= '<g:additional_image_link>' . $this->model_tool_image->resize(
                                        $image,
                                        $this->config->get($this->config->get('config_theme') . '_image_popup_width'),
                ...
                if ($product['special'] && $product['special'] < $product['price']) {
                    $xml .= '<g:price>' . number_format(
                            $this->currency->convert(
                                $this->tax->calculate($product['price'] * $multiplier, $product['tax_class_id']),
                                $shop_currency,
                                $items_currency
                            ),
                            $decimal,
                            '.',
                            ''
                        ) . ' ' . $items_currency . '</g:price>' . $this->eol;
                    $xml .= '<g:sale_price>' . number_format(
                            $this->currency->convert(
                                $this->tax->calculate($product['special'] * $multiplier, $product['tax_class_id']),
                                $shop_currency,
                                $items_currency
                            ),
                            $decimal,
                            '.',
                            ''
                        ) . ' ' . $items_currency . '</g:sale_price>' . $this->eol;
                    if ((!empty($product['special_date_end']) && strtotime($product['special_date_end']) > time(
                            )) || $product['special_date_end'] != '0000-00-00') {
                        $xml .= '<g:sale_price_effective_date>' . date(
                                'c',
                                strtotime($product['special_date_end'])
                            ) . '</g:sale_price_effective_date>' . $this->eol;
                    }
                } else {
                    $xml .= '<g:price>' . number_format(
                            $this->currency->convert(
                ...
                if ($auto_min_price) {
                    $xml .= '<g:auto_pricing_min_price>' . number_format(
                            $this->currency->convert(
                                $this->tax->calculate(
                                    (($product['special'] && $product['special'] < $product['price']) ? $product['special'] : $product['price']) * $multiplier * 0.95,
                            ...
                            '.',
                            ''
                        ) . ' ' . $items_currency . '</g:auto_pricing_min_price>' . $this->eol;
                }
                ...
                if ($type_category || $esputnik) {
                    $xml .= '<g:product_type>' . $this->getRemarketingFeedCategories(
                            $product['product_id'],
                            $ocstore
                        ) . '</g:product_type>' . $this->eol;
                }
    ...
    private function getImages($limit = 10)
    {
        $images = [];
        $query = $this->db->query(
            "SELECT pi.product_id, pi.image FROM `" . DB_PREFIX . "product_image` pi ORDER BY pi.product_id, pi.sort_order"
        );
    ...
    private function getLastCategory($product_id)
    {
        $category_query = $this->db->query(
            "SELECT pc.category_id FROM `" . DB_PREFIX . "product_to_category` pc LEFT JOIN `" . DB_PREFIX . "category_path` cp ON pc.category_id = cp.category_id WHERE pc.product_id = '" . (int)$product_id . "' ORDER BY cp.level DESC LIMIT 1"
        );
    ...
    public function googleReviews()
    {
        if ($this->config->get('remarketing_reviews_status')) {
            ...
            $output .= '<feed xmlns:vc="http://www.w3.org/2007/XMLSchema-versioning" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.google.com/shopping/reviews/schema/product/2.2/product_reviews.xsd"><version>2.2</version><aggregator><name>Opencart</name></aggregator><publisher><name>' . $this->config->get(
                    'config_name'
                ) . '</name><favicon>' . $this->config->get('config_ssl') . 'image/' . $this->config->get(
                    'config_icon'
                ) . '</favicon></publisher>';
            ...
            foreach ($reviews as $review) {
                ...
                if ($product) {
                    ...
                    $output .= '<reviewer><name' . ($this->config->get(
                            'remarketing_reviews_feed_anonymous'
                        ) ? ' is_anonymous="true"' : '') . '>' . $review['author'] . '</name></reviewer>';
                    $output .= '<review_timestamp>' . date(
                            'Y-m-d\TH:i:sP',
                            strtotime($review['date_added'])
                        ) . '</review_timestamp>';
                    $output .= '<content>' . $review['text'] . '</content>';
                    $output .= '<is_spam>false</is_spam>';
                    $output .= '<review_url type="group">' . $this->url->link(
                            'product/product',
                            'product_id=' . $review['product_id']
                        ) . '</review_url>';
                    $output .= '<ratings><overall min="1" max="5">' . $review['rating'] . '</overall></ratings>';
                    $output .= '<products><product><product_url>' . $this->url->link(
                            'product/product',
                            'product_id=' . $review['product_id']
                        ) . '</product_url><product_name>' . $this->checkField($review['name']) . '</product_name>';
                    $output .= '<product_ids>';
                    ...
                    if ($this->config->get('remarketing_reviews_feed_gtin')) {
                        if (!empty($product[$this->config->get('remarketing_reviews_feed_gtin')])) {
                            $output .= '<gtins><gtin>' . $product[$this->config->get(
                    ...
                    if ($this->config->get('remarketing_reviews_feed_mpn')) {
                        if (!empty($product[$this->config->get('remarketing_reviews_feed_mpn')])) {
                            $output .= '<mpns><mpn>' . $product[$this->config->get(
                    ...
                    if ($this->config->get('remarketing_reviews_feed_sku')) {
                        if (!empty($product[$this->config->get('remarketing_reviews_feed_sku')])) {
                            $output .= '<skus><sku>' . $product[$this->config->get(
                    ...
                    if ($this->config->get('remarketing_reviews_feed_asin')) {
                        if (!empty($product[$this->config->get('remarketing_reviews_feed_asin')])) {
                            $output .= '<asins><asin>' . $product[$this->config->get(
    ...
    {
        $sql = "SELECT pd.*, r.*, r.review_id, pd.name, r.author, r.rating, r.status, r.date_added FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "'";
    ...
    public function getRemarketingFeedCategories($product_id, $ocstore = false)
    {
        ...
        $category_query = $this->db->query(
            "SELECT DISTINCT cd.name FROM `" . DB_PREFIX . "product_to_category` pc LEFT JOIN `" . DB_PREFIX . "category_path` cp ON pc.category_id = cp.category_id LEFT JOIN `" . DB_PREFIX . "category_description` cd ON cp.path_id = cd.category_id WHERE pc.product_id = '" . (int)$product_id . "' AND cd.language_id = '" . (int)$this->config->get(
                'config_language_id'
    ...
    public function getAttributes($product_id, $allowed_attributes)
    {
        ...
        $product_attribute_query = $this->db->query(
            "SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE pa.product_id = '" . (int)$product_id . "' AND ad.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "' AND pa.language_id = '" . (int)$this->config->get(
                'config_language_id'
    ...
    public function getColorOptions($product_id, $color_options = [])
    {
        ...
        $option_query = $this->db->query(
            "SELECT *, od.name as option_name, ovd.name as option_value FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (pov.option_value_id = ovd.option_value_id AND ovd.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "') LEFT JOIN " . DB_PREFIX . "option_description od ON (pov.option_id = od.option_id AND od.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "') WHERE pov.product_id = '" . (int)$product_id . "' AND pov.option_id IN (" . implode(
                ',',
                $color_options
            ) . ")"
        );
    ...
    public function getFastAttributes($product_id, $allowed_attributes)
    {
        ...
        $product_attribute_query = $this->db->query(
            "SELECT pa.attribute_id, pa.text FROM " . DB_PREFIX . "product_attribute pa WHERE pa.product_id = '" . (int)$product_id . "' AND pa.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "' AND pa.attribute_id IN (" . implode(',', $allowed_attributes) . ")"
        );
    ...
    public function getProductAttributes($product_id)
    {
        ...
        $product_attribute_group_query = $this->db->query(
            "SELECT ag.attribute_group_id, agd.name FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE pa.product_id = '" . (int)$product_id . "' AND agd.language_id = '" . (int)$this->config->get(
                'config_language_id'
        ...
        foreach ($product_attribute_group_query->rows as $product_attribute_group) {
            ...
            $product_attribute_query = $this->db->query(
                "SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE pa.product_id = '" . (int)$product_id . "' AND a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND ad.language_id = '" . (int)$this->config->get(
                    'config_language_id'
                ) . "' AND pa.language_id = '" . (int)$this->config->get(
                    'config_language_id'
    ...
    private function getCsv($page = 0)
    {
        ...
        foreach ($this->model_tool_remarketing_core->getProducts($start, $limit, $ocstore) as $product) {
            ...
            if ($original_images) {
                ...
                if (version_compare(VERSION, '3.0.0.0', '>=')) {
                    $img = $this->model_tool_image->resize(
                        $product['image'],
