<?php
class ControllerExtensionFeedUnixmlAliexpress extends Controller {
        ...
        for($startup['iteration'] = 0; 1; $startup['iteration']++){
          ...
          if($controller_data['products']){
            ...
            foreach($controller_data['products'] as $product_id => $product){
              ...
              if($product['special']){
                $xml .= '<discount_price>' . $product['special'] .  '</discount_price>';
              ...
              $xml .= '<quantity>' . ($product['quantity']?$product['quantity']:'0') .  '</quantity>';
