<?php
class ControllerExtensionFeedUnixmlOzon extends Controller {
        ...
        for($startup['iteration'] = 0; 1; $startup['iteration']++){
          ...
          if($controller_data['products']){
            ...
            foreach($controller_data['products'] as $product_id => $product){
              ...
              if(isset($product['premium']) && $product['premium']){
                $xml .= '<premium_price>' . $product['premium'] .  '</premium_price>';
              }

              if(isset($product['outlets']) && $product['outlets']){
                $xml .= '<outlets>';
                  foreach($product['outlets'] as $outlet){
                    $xml .= '<outlet instock="' . $outlet['stock'] . '" warehouse_name="' . $outlet['name'] . '"></outlet>';
                  }
                $xml .= '</outlets>';
              }
