...
class ModelReportRemarketing extends Model
{
    ...
    public function getUtm($utm = '')
    {
        $utms = [];
        $query = $this->db->query(
            "SELECT DISTINCT(" . $this->db->escape($utm) . ") FROM `" . DB_PREFIX . "remarketing_orders` ro"
        );
    ...
    {
        $sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get(
                'config_language_id'
            ) . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";

        if (!empty($data['filter_order_ids'])) {
            $sql .= " WHERE o.order_id IN (" . $data['filter_order_ids'] . ")";
        }
    ...
    {
        $sql = "SELECT op.product_id, SUM(op.quantity) as product_quantity, SUM(op.total) as product_total FROM " . DB_PREFIX . "order_product op WHERE op.order_id IN (" . $data['filter_order_ids'] . ") GROUP BY op.product_id  ORDER BY product_total DESC";
