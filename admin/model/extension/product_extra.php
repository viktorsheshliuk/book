...
class ModelExtensionProductExtra extends Model {
	...
	public function getAlias($product_id){
		$row = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE query = 'product_id=" . (int)$product_id. "'");
	...
	public function changeSeo($product_id, $keyword, $language = null){
		...
		if ($keyword != '') {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->db->escape($keyword) . "', language_id = '".$selected_language."', store_id = '0'");
		}
		$this->cache->delete('product');
