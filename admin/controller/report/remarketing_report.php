    ...
    public function index()
    {
        ...
        foreach ($results as $result) {
            $data['orders'][] = array(
                ...
                'details' => $this->url->link(
                    'report/remarketing_report/details',
                    $token . '&filter_order_ids=' . $result['order_ids'],
                    true
                ),
        ...
        $data['results'] = sprintf(
            ...
            ((($page - 1) * $this->config->get('config_limit_admin')) > ($order_total - $this->config->get(
                        'config_limit_admin'
                    ))) ? $order_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get(
                    'config_limit_admin'
                )),
            $order_total,
    ...
    public function details()
    {
        ...
        foreach ($results as $result) {
            $data['orders'][] = [
                ...
                'order_status' => $result['order_status'] ? $result['order_status'] : $this->language->get(
                    'text_missing'
                ),
        ...
        foreach ($results as $result) {
            ...
            $data['products'][] = [
                ...
                'edit' => $this->url->link(
                    ...
                    $token . '&product_id=' . $result['product_id'],
                    true
                )
            ];
        }
