(function ( $ ) {
    ...
    function updateInputValues(inputBinding, gmapContext){
        ...
        if (inputBinding.latlongInput) {
            inputBinding.latlongInput.val(currentLocation.latitude + ',' + currentLocation.longitude).change();
        }
    }
    ...
    function setupInputListenersInput(inputBinding, gmapContext) {
        if (inputBinding) {
            ...
            if (inputBinding.latlongInput) {
                inputBinding.latlongInput.on("change", function(e) {
                    if (!e.originalEvent) { return }
                    var ll = $(this).val().split(",");
                    GmUtility.setPosition(gmapContext, new google.maps.LatLng(ll[0], ll[1]), function(context){
                        context.settings.onchanged.apply(gmapContext.domContainer,
                            [GmUtility.locationFromLatLng(context.location), context.radius, false]);
                    });
                });
